...
export const CardSkeleton = () => (
  <div className="aspect-9/16 rounded-2xl bg-gray-200 relative overflow-hidden">
    <div className="absolute inset-0 bg-linear-to-r from-transparent via-white/40 to-transparent -skew-x-12 translate-x-[-150%] animate-shimmer" />
  </div>
);
...
export const StoryCircle = ({
  ...
  return (
    <div
      onClick={onClick}
      className="flex flex-col items-center gap-2 cursor-pointer shrink-0 group w-[72px]"
    >
      <div className="relative w-[68px] h-[68px] transition-transform duration-300 group-hover:scale-105 group-active:scale-95">
        ...
        >
          <div className="bg-white rounded-full p-0.5 w-full h-full overflow-hidden">
            <img
...
export const TrendingCard = ({ story, onClick }) => {
  return (
    <div
      onClick={onClick}
      className="group relative aspect-9/16 rounded-2xl overflow-hidden cursor-pointer bg-gray-100 shadow-sm hover:shadow-xl transition-all duration-300"
    >
      ...
      <div className="absolute bottom-0 left-0 w-full p-4 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
        <div className="flex items-center gap-2 mb-1">
          <img
            src={story.avatar || "https://ui-avatars.com/api/?name=User"}
      ...
      <div className="relative w-full h-full md:w-[400px] md:h-[90vh] md:rounded-3xl bg-black overflow-hidden shadow-2xl z-10 ring-1 ring-white/10">
        ...
        <div className="absolute top-4 left-3 right-3 z-30 flex gap-1.5 h-1">
          {playlist.map((_, idx) => (
            <div
              key={idx}
              className="flex-1 bg-white/20 rounded-full overflow-hidden h-full"
            >
              <div
        ...
        <div className="absolute top-8 left-4 z-30 flex items-center gap-3">
          <img
            src={currentStory.avatar || "/default.png"}
        ...
        <button
          onClick={onClose}
          className="absolute top-8 right-4 z-30 text-white/80 hover:text-white p-2"
        >
          <X size={24} />
        </button>
        ...
        {/* Footer */}
        <div className="absolute bottom-0 w-full p-4 z-30 bg-linear-to-t from-black/90 to-transparent pb-8 pt-12">
          <div className="flex gap-4 items-center">
            <input
              type="text"
              placeholder="Send message..."
              className="bg-white/10 border border-white/20 rounded-full px-4 py-2.5 text-white text-sm w-full focus:outline-none focus:bg-white/20 placeholder-white/50 backdrop-blur-sm"
            />
            <button
