...
export default function StoryCreator({ onClose, currentUser }) {
  ...
  const takePhoto = async () => {
    ...
    canvasRef.current.toBlob(
      async (blob) => await uploadToFirebase(blob, "image"),
      "image/jpeg",
      0.95
    );
  };
        ...
        {(isUploading || !cameraReady) && (
          <div className="absolute inset-0 bg-black/80 backdrop-blur-xl flex flex-col items-center justify-center z-50">
            ...
            <p className="font-semibold tracking-wide">
              {!cameraReady ? "Loading AR Engine..." : "Sharing Moment..."}
            </p>
          </div>
        )}
        ...
        <div className="absolute bottom-0 w-full z-20 pb-8 pt-12 bg-linear-to-t from-black/90 via-black/50 to-transparent flex flex-col items-center">
          <div className="w-full overflow-x-auto scrollbar-hide mb-6 px-4 snap-x">
            <div className="flex gap-4 items-center justify-center min-w-max px-4">
              {FILTERS.map((f) => (
                ...
                >
                  <div
                    className={`w-14 h-14 rounded-full flex items-center justify-center border-2 backdrop-blur-md shadow-lg ${
                      selectedFilter === f.id
                        ? `${f.color} border-white text-white`
                  ...
                  <span
                    className={`text-[10px] font-bold uppercase tracking-wider ${
                      selectedFilter === f.id ? "opacity-100" : "opacity-0"
                    }`}
                  >
                    {f.name}
                  </span>
          ...
          <div className="relative mb-2">
            <svg className="w-20 h-20 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-90 pointer-events-none">
              ...
              {isRecording && (
                <circle
                  ...
                  fill="transparent"
                  strokeDasharray="238"
                  strokeDashoffset={238 - (238 * recordingProgress) / 100}
                  strokeLinecap="round"
            ...
            <button
              ...
              className={`w-16 h-16 rounded-full border-4 border-transparent bg-white shadow-2xl transition-all duration-200 ${
                isRecording
                  ? "scale-75 bg-red-500"
